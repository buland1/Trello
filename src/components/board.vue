    ...
    <v-row>
      ...
      <v-col md="4">
        <v-card class="mx-auto" max-width="330">
          ...
          <h5 v-if="onProgressList.length === 0 || onProgressList === null">
            <div>
              <p class="w-100" style="background-color: #eee">
                Not Listed, You can switch cards
              </p>
            </div>
          </h5>
      ...
      <v-col md="4">
        <v-card class="mx-auto" max-width="330">
          ...
          <h5 v-if="completedList.length === 0 || completedList === null">
            <div>
              <p class="w-100" style="background-color: #eee">
                Not Listed, You can switch cards
              </p>
    ...
    <v-dialog v-model="dialog" persistent max-width="400px">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn
            color="blue darken-1"
            :disabled="status.length < 1 || description.length < 3"
            text
            @click="saveField()"
          >
            Save
  ...
  methods: {
    ...
    clickArrow({ cardIndex, moveTo }) {
      if (moveTo === 0) {
        ...
        let deletedList = this.issueList.splice(cardIndex, 1);
    ...
    clickArrow1({ cardIndex, moveTo }) {
      if (moveTo === 0) {
        ...
        console.log("Delete Button should be enabled");
        let deletedList = this.onProgressList.splice(cardIndex, 1);
        console.log(deletedList)
        localStorage.setItem("onProgressList",
          JSON.stringify(this.onProgressList)
        );
      }
    },
    clickArrow2({ cardIndex, moveTo }) {
      if (moveTo === 0) {
        ...
        console.log("Delete Button should be enabled");
        let deletedList = this.completedList.splice(cardIndex, 1);
        console.log(deletedList)
